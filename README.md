...
When you hit the limit you receive a `429 Too Many Requests` response. Back off for the number of seconds in `Retry-After` before retrying. If you're polling for new mail, use `GET /v1/inbox/{address}/wait` (long-poll) or a webhook instead of a tight loop. Both cost far less against this limit.
...
Long-polls for new emails. Returns immediately if emails already exist, otherwise blocks up to the given timeout waiting for one. **Costs 1 credit per request**, the same as `GET /v1/inbox/{address}`, but far more efficient than tight polling: it counts once against your rate limit no matter how long it waits.
...
Clear all emails from an inbox. Free, never charges a credit.
...
Register a webhook and VanishInbox POSTs to your URL the moment an email arrives, usually within a second of delivery. No polling required.
...
**Payload.** Deliveries carry the full email: sender, subject, HTML and plain-text body, timestamp. You won't need a follow-up call to `GET /v1/inbox/{address}` to read what arrived. The body goes to whatever URL you register, so point `target_url` only at an endpoint you trust.
...
`target_url` must be `https`, publicly resolvable, and not point at a private, loopback, or link-local address.
...
> `secret` comes back **once**, at registration. Store it; we can't show it again. If you lose it, register a new webhook for a fresh one.
...
```json
{
  ...
  "subject": "Confirm your email address",
  "body": "<p>Your verification code is <strong>482910</strong></p>",
...
**Prefer `/wait` or webhooks over polling.** Both cost the same as a single `GET`. `/wait` blocks server-side for up to 60 seconds; a webhook pushes the moment mail lands. Either beats a `sleep`-and-retry loop on credits and latency.
